...
void ofApp::setup() {
  ...
  m_earthTexture.load("textures/4096_earth_clouds.png");
    m_concreteTexture.load("textures/concrete.jpg");

    
    
    gui.setup("gui");
    gui.add(rotSpeed.set("rot speed", 0.0, 0.0, 3.0));
    gui.add(orbitSpeed.set("orbit", 0.0, 0.0, 1.0));
    gui.add(ambient.set("ambient", ofFloatColor(0.5, 0.5, 0.5), ofFloatColor(0.0, 0.0, 0.0), ofFloatColor(1.0, 1.0, 1.0)));
    gui.add(diffuse.set("diffuse", ofFloatColor(0.5, 0.5, 0.5), ofFloatColor(0.0, 0.0, 0.0), ofFloatColor(1.0, 1.0, 1.0)));
    gui.add(specular.set("specular", ofFloatColor(0.5, 0.5, 0.5), ofFloatColor(0.0, 0.0, 0.0), ofFloatColor(1.0, 1.0, 1.0)));
    gui.add(attConstant.set("att constant", 0.0, 0.0, 1.0));
    gui.add(attLinear.set("attLinear", 0.0, 0.0, 1.0));
    gui.add(attExp.set("attExp", 0.0, 0.0, 1.0));
    float y = ofGetHeight() - gui.getHeight() - 10;
    gui.setPosition(10, y);
  ...
  ofMesh boxMesh = Primitives::getSphereMesh(6);
  m_numBoxVerts = boxMesh.getNumVertices();
  m_boxVbo.setMesh(boxMesh, GL_STATIC_DRAW);
    
  
    ofMesh earthMesh = Primitives::getSphereMesh(30);
    m_numSphereEarthVerts = earthMesh.getVertices().size();
    m_earthVbo.setMesh(earthMesh, GL_STATIC_DRAW);
...
void ofApp::addRandomLight() {
  ...
  l.setAmbient(ambient->r, ambient->g, ambient->b);

//  ofVec3f col = ofVec3f(ofRandom(0.3f, 0.5f), ofRandom(0.2f, 0.4f), ofRandom(0.7f, 1.0f));
  l.setDiffuse(diffuse->r, diffuse->g, diffuse->b);
  l.setSpecular(specular->r, specular->g, specular->b);
  l.setAttenuation(attConstant, attLinear, attExp); // set constant, linear, and exponential attenuation
  l.intensity = 0.8f;
...
void ofApp::geometryPass() {
  ...
  m_earthTexture.getTexture().bind();
...
//  for (vector<Box>::iterator it=m_boxes.begin() ; it < m_boxes.end(); it++) {
    ofPushMatrix();
    ofRotate(earthRotDeg+=rotSpeed, 0,1,0);
//    ofRotate(it->angle, it->axis_x, it->axis_y, it->axis_z);
//    ofScale(it->size, it->size, it->size);
    ofScale(17,17,17);
    glDrawArrays(GL_TRIANGLE_STRIP, 0, m_numSphereEarthVerts);
    ofPopMatrix();
//  }
  ...
  m_earthVbo.unbind();
    ...
    m_earthTexture.getTexture().unbind();
    
   
    
    
    
    m_concreteTexture.getTexture().bind();
  ...
  m_sphereVbo.unbind();
    m_concreteTexture.getTexture().unbind();
...
void ofApp::update() {
  ...
  for (vector<PointLight>::iterator it = m_lights.begin(); it != m_lights.end(); it++) {
    ...
    it->rotateAround(percent * orbitSpeed + 0.1f, it->orbitAxis, ofVec3f(0.0f, 0.0f, 0.0f));

      
      it->setAmbient(ambient->r, ambient->g, ambient->b);
      
      //  ofVec3f col = ofVec3f(ofRandom(0.3f, 0.5f), ofRandom(0.2f, 0.4f), ofRandom(0.7f, 1.0f));
      it->setDiffuse(diffuse->r, diffuse->g, diffuse->b);
      it->setSpecular(specular->r, specular->g, specular->b);
      it->setAttenuation(attConstant, attLinear, attExp); // set constant, linear, and exponential
